...
import {DegenWethStackFixture} from "./DegenWethStack.t.sol";

contract DegenWethInvariantHandler {
    IDegenV3LpLocker public immutable locker;
...
contract DegenWethStackInvariant is StdInvariant, DegenWethStackFixture {
    DegenWethInvariantHandler private handler;

    function setUp() public {
        _setUpDegenWethStack();
        module.configure(_context());
        handler = new DegenWethInvariantHandler(
            lpLocker,
            address(token),
        ...
        IERC20(weth).transfer(address(handler), 1 ether);
        vm.prank(feeAdmin);
    ...
    function invariantSupplyDustAndFeeBackingRemainConservative() public view {
        ...
        assertGe(IERC20(weth).balanceOf(address(feeLocker)), feeLocker.totalLiability());
    }
    ...
    function _context() private view returns (LaunchContext memory) {
        return LaunchContext({
            ...
            commitment: keccak256("production-weth-invariant-launch"),
            token: address(token),
