...
contract DegenSpyV3LpLocker is IDegenSpyV3LpLocker, ReentrancyGuard {
    using PoolIdLibrary for PoolKey;
    using PositionInfoLibrary for PositionInfo;
    using SafeERC20 for IERC20;

    uint256 public constant POOL_SUPPLY = 100_000_000_000 ether;
    uint256 public constant RATE_DENOMINATOR = DegenSpyV3LaunchConstants.RATE_DENOMINATOR;
        ...
        for (uint256 i; i < DegenSpyV3LaunchConstants.TRANCHE_COUNT; ++i) {
            ...
            if (
                PoolId.unwrap(positionKey.toId()) != PoolId.unwrap(poolKey.toId())
                    || info.tickLower() != DegenSpyV3LaunchConstants.lowerTick(i)
                    || info.tickUpper() != DegenSpyV3LaunchConstants.upperTick(i)
                    || positionManager.getPositionLiquidity(positionIds[i])
                        != DegenSpyV3LaunchConstants.liquidity(poolSupply, i)
            ) revert InvalidPositionReceipt();
        }
    ...
    function collectRewards(address token)
        external
        nonReentrant
        returns (uint256 tokenFees, uint256 spyFees)
    {
        ...
        returns (uint256 beneficiaryRawSpyDelivered)
    {
        ...
        (,, uint256 hookCreatorRawSpyStored) =
            IDegenSpyV3Hook(hook).flushPoolFees(config.poolKey.toId(), beneficiary);
    ...
    function updateBeneficiary(address token, address newBeneficiary) external nonReentrant {
        ...
        IDegenSpyV3Hook degenHook = IDegenSpyV3Hook(hook);
        ...
        returns (uint256 tokenFees, uint256 spyFees)
    {
        ...
        bytes memory actions = new bytes(DegenSpyV3LaunchConstants.TRANCHE_COUNT + 1);
        bytes[] memory params = new bytes[](DegenSpyV3LaunchConstants.TRANCHE_COUNT + 1);
        for (uint256 i; i < DegenSpyV3LaunchConstants.TRANCHE_COUNT; ++i) {
            actions[i] = bytes1(uint8(Actions.DECREASE_LIQUIDITY));
            params[i] = abi.encode(config.positionIds[i], 0, 0, 0, bytes(""));
        }
        actions[DegenSpyV3LaunchConstants.TRANCHE_COUNT] = bytes1(uint8(Actions.TAKE_PAIR));
        params[DegenSpyV3LaunchConstants.TRANCHE_COUNT] =
        ...
        spyFees = quoteToken.balanceOf(address(this)) - spyBalanceBefore;
        ...
        if (spyFees != 0) {
            quoteToken.forceApprove(address(feeLocker), spyFees);
            uint256 stored = feeLocker.storeFees(config.beneficiary, spyFees);
            quoteToken.forceApprove(address(feeLocker), 0);
            if (stored != spyFees) revert UnsupportedTokenBehavior();
        }
        emit FeesCollected(
            token, config.beneficiary, tokenFees, tokenReserveAmount, tokenBurnAmount, spyFees
        );
    }
        ...
        view
    {
        ...
        IDegenSpyV3Hook.PoolConfig memory hookConfig =
            IDegenSpyV3Hook(hook).getPoolConfig(poolKey.toId());
        if (
