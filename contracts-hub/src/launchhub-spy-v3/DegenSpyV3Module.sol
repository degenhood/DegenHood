...
import {IDegenSpyV3LpLocker} from "../interfaces/IDegenSpyV3LpLocker.sol";
import {IDegenSpyV3Module} from "../interfaces/IDegenSpyV3Module.sol";
...
interface IDegenSpyV3HookBinding {
...
interface IDegenSpyV3LpLockerBinding {
    function module() external view returns (address);
...
/// @title SPY V3 LaunchHub module
/// @notice Creates the ten-position SPY pool using PositionManager-owned direct settlement.
contract DegenSpyV3Module is IDegenSpyV3Module {
    ...
    IDegenSpyV3Hook public immutable hook;
    IDegenSpyV3LpLocker public immutable lpLocker;
    ...
    constructor(
        ...
        IDegenSpyV3HookBinding hookBinding = IDegenSpyV3HookBinding(hook_);
        IDegenSpyV3LpLockerBinding lockerBinding = IDegenSpyV3LpLockerBinding(lpLocker_);
        ...
        if (
            hookBinding.module() != address(this) || lockerBinding.module() != address(this)
                ...
                || (block.chainid == ROBINHOOD_CHAIN_ID
                    && quoteToken != DegenSpyV3LaunchConstants.SPY)
        ...
        hook = IDegenSpyV3Hook(hook_);
        lpLocker = IDegenSpyV3LpLocker(lpLocker_);
        poolManager = manager;
        ...
        returns (LaunchResult memory result)
    {
        ...
        bytes memory actions = new bytes(DegenSpyV3LaunchConstants.TRANCHE_COUNT + 3);
        bytes[] memory params = new bytes[](DegenSpyV3LaunchConstants.TRANCHE_COUNT + 3);
        for (uint256 i; i < DegenSpyV3LaunchConstants.TRANCHE_COUNT; ++i) {
            positionIds[i] = firstPositionId + i;
            actions[i] = bytes1(uint8(Actions.MINT_POSITION));
            uint256 trancheSupply = DegenSpyV3LaunchConstants.trancheSupply(POOL_SUPPLY, i);
            params[i] = abi.encode(
                poolKey,
                DegenSpyV3LaunchConstants.lowerTick(i),
                DegenSpyV3LaunchConstants.upperTick(i),
                uint256(DegenSpyV3LaunchConstants.liquidity(POOL_SUPPLY, i)),
        ...
        uint256 settleIndex = DegenSpyV3LaunchConstants.TRANCHE_COUNT;
    ...
    function _validateSpyTransferSurface(address beneficiary) private view returns (uint256) {
        ...
        if (observedUid != DegenSpyV3LaunchConstants.SPY_UID) {
        ...
        if (
            registryAddress.code.length == 0
                || (block.chainid == 4663
                    && registryAddress != DegenSpyV3LaunchConstants.SPY_REGISTRY)
        ...
        IDegenSpyV3HookBinding hookBinding = IDegenSpyV3HookBinding(address(hook));
        ...
        _requireNotBlocked(registry, address(positionManager));
