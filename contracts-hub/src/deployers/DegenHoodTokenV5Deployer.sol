...
import {ITokenDeployer, TokenArgs} from "../LaunchHub.sol";
...
interface IDegenHoodTokenV5ModuleBinding {
    function poolManager() external view returns (address);
    function positionManager() external view returns (address);
    function lpLocker() external view returns (address);
}

/// @title DegenHoodTokenV5Deployer
/// @notice Kernel-only CREATE2 deployer for implementation-bound ERC-1167 token clones.
contract DegenHoodTokenV5Deployer is ITokenDeployer {
    error InvalidKernel();
        ...
        returns (address token)
    {
        ...
        IDegenHoodTokenV5ModuleBinding moduleBinding = IDegenHoodTokenV5ModuleBinding(args.module);
    ...
    function _isLockedImplementation(address implementation_) private view returns (bool) {
        ...
        (bool initialisedOk, bytes memory initialisedData) =
            implementation_.staticcall(abi.encodeCall(IDegenHoodTokenV5.initialised, ()));
        (bool frozenOk, bytes memory frozenData) =
            implementation_.staticcall(abi.encodeCall(IDegenHoodTokenV5.metadataFrozen, ()));
