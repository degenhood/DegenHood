...
import {IDegenV1Module} from "../interfaces/IDegenV1Module.sol";

interface IDegenV1HookBinding {
    function module() external view returns (address);
    function poolManager() external view returns (IPoolManager);
    function feeLocker() external view returns (address);
}

interface IDegenV1LpLockerBinding {
    function module() external view returns (address);
    function hook() external view returns (address);
    function feeLocker() external view returns (address);
}

/// @title DEGEN_V1 LaunchHub module
/// @notice Atomically configures the canonical pool and permanently locks the complete token supply.
contract DegenV1Module is IDegenV1Module {
    ...
    IDegenV1Hook public immutable hook;
    IDegenV1LpLocker public immutable lpLocker;
    ...
    constructor(
        ...
        if (
            IDegenV1HookBinding(hook_).module() != address(this)
                || IDegenV1LpLockerBinding(lpLocker_).module() != address(this)
                || IDegenV1LpLockerBinding(lpLocker_).hook() != hook_
                || IDegenV1HookBinding(hook_).feeLocker()
                    != IDegenV1LpLockerBinding(lpLocker_).feeLocker()
        ) {
            revert InvalidChildBinding();
        }

        IPoolManager manager = IDegenV1HookBinding(hook_).poolManager();
        if (address(manager) == address(0)) revert InvalidChildBinding();

        kernel = kernel_;
        domainId = domainId_;
        configHash = configHash_;
        hook = IDegenV1Hook(hook_);
        lpLocker = IDegenV1LpLocker(lpLocker_);
        poolManager = manager;
    }
