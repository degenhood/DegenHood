...
/// @title LaunchHub V2
/// @notice Independent two-template V2 kernel; the V1 deployment and its records remain immutable.
contract LaunchHubV2 {
    using SignatureChecker for address;
    using SafeCast for uint256;

    bytes32 public constant KERNEL_VERSION = keccak256("LaunchHub/2.0.0");
    bytes32 public constant EMPTY_SCHEMA_HASH = keccak256("EMPTY");
    uint256 public constant DOMAIN_ADMISSION_DELAY = 72 hours;
    TemplateId public constant DEGEN_V2_UNISWAP_V4 = TemplateId.wrap(2);
    TemplateId public constant DEGEN_V2_SPY_UNISWAP_V4 = TemplateId.wrap(4);
    ...
    bytes32 private constant NAME_HASH = keccak256("DegenHoodLaunchHubV2");
    ...
    constructor(
        ...
        if (
            TemplateId.unwrap(templates[0].id) != TemplateId.unwrap(DEGEN_V2_UNISWAP_V4)
                || TemplateId.unwrap(templates[1].id) != TemplateId.unwrap(DEGEN_V2_SPY_UNISWAP_V4)
        ) {
    ...
    function _installGenesisTemplate(DomainId domainId, GenesisTemplate memory genesis) private {
        if (Version.unwrap(genesis.version) != 2) revert InvalidGenesisVersion();
