...
/// @title DegenSpyV3LaunchConstants
/// @notice Immutable raw-SPY V3 economics and ten-position launch curve.
library DegenSpyV3LaunchConstants {
    ...
    function lowerTick(uint256 tranche) internal pure returns (int24) {
        ...
        if (tranche == 1) return -224_200;
        if (tranche == 2) return -216_200;
        if (tranche == 3) return -208_200;
        if (tranche == 4) return -203_200;
        if (tranche == 5) return -190_200;
        if (tranche == 6) return -165_000;
        if (tranche == 7) return -144_400;
        if (tranche == 8) return -123_800;
        if (tranche == 9) return -103_000;
        revert InvalidTranche(tranche);
    }

    function upperTick(uint256 tranche) internal pure returns (int24) {
        if (tranche == 0) return -224_200;
        if (tranche == 1) return -216_200;
        if (tranche == 2) return -208_200;
        if (tranche == 3) return -203_200;
        if (tranche == 4) return -190_200;
        if (tranche == 5) return -165_000;
        if (tranche == 6) return -144_400;
        if (tranche == 7) return -123_800;
        if (tranche == 8) return -103_000;
