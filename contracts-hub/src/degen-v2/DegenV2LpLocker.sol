...
import {IDegenV2LpLocker} from "../interfaces/IDegenV2LpLocker.sol";
import {DegenV2LaunchConstants} from "../libraries/DegenV2LaunchConstants.sol";

/// @title DEGEN_V2 permanent six-position LP locker
/// @notice Places the complete launch supply permanently and routes measured LP fee deltas.
/// @dev Token-side LP fees are split 80% to the dead address and 20% to the immutable reserve.
/// Quote-side LP fees and the isolated hook creator bucket are checkpointed then paid to the
/// immutable creator beneficiary. There is no NFT, principal, rescue, sweep, or arbitrary-call exit.
contract DegenV2LpLocker is IDegenV2LpLocker, ReentrancyGuard {
    using PoolIdLibrary for PoolKey;
    using SafeERC20 for IERC20;

    uint256 public constant POOL_SUPPLY = 100_000_000_000 ether;
    uint256 public constant RATE_DENOMINATOR = DegenV2LaunchConstants.RATE_DENOMINATOR;
    ...
    function placeLiquidity(
        ...
        for (uint256 i; i < DegenV2LaunchConstants.TRANCHE_COUNT; ++i) {
            uint256 trancheSupply =
                poolSupply * DegenV2LaunchConstants.supplySharePips(i) / RATE_DENOMINATOR;
            int24 tickLower = DegenV2LaunchConstants.lowerTick(i);
            int24 tickUpper = DegenV2LaunchConstants.upperTick(i);
        ...
        returns (uint256 beneficiaryWethDelivered)
    {
        ...
        (,, uint256 hookCreatorWethStored) =
            IDegenV2Hook(hook).flushPoolFees(config.poolKey.toId(), beneficiary);
    ...
    function updateBeneficiary(address token, address newBeneficiary) external nonReentrant {
        ...
        IDegenV2Hook degenHook = IDegenV2Hook(hook);
        ...
        view
    {
        ...
        IDegenV2Hook.PoolConfig memory hookConfig = IDegenV2Hook(hook).getPoolConfig(poolKey.toId());
        if (
