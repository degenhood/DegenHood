...
import {IDegenV2Module} from "../interfaces/IDegenV2Module.sol";
import {DegenV2LaunchConstants} from "../libraries/DegenV2LaunchConstants.sol";

interface IDegenV2HookBinding {
    function module() external view returns (address);
    function poolManager() external view returns (IPoolManager);
    function feeLocker() external view returns (address);
}

interface IDegenV2LpLockerBinding {
    function module() external view returns (address);
    function hook() external view returns (address);
    function feeLocker() external view returns (address);
}

/// @title DEGEN_V2 LaunchHub module
/// @notice Atomically creates the canonical ~10 WETH six-position pool and permanent custody.
contract DegenV2Module is IDegenV2Module {
    ...
    IDegenV2Hook public immutable hook;
    IDegenV2LpLocker public immutable lpLocker;
    ...
    constructor(
        ...
        if (
            IDegenV2HookBinding(hook_).module() != address(this)
                || IDegenV2LpLockerBinding(lpLocker_).module() != address(this)
                || IDegenV2LpLockerBinding(lpLocker_).hook() != hook_
                || IDegenV2HookBinding(hook_).feeLocker()
                    != IDegenV2LpLockerBinding(lpLocker_).feeLocker()
        ) {
            revert InvalidChildBinding();
        }

        IPoolManager manager = IDegenV2HookBinding(hook_).poolManager();
        if (address(manager) == address(0)) revert InvalidChildBinding();
        kernel = kernel_;
        domainId = domainId_;
        configHash = configHash_;
        hook = IDegenV2Hook(hook_);
        lpLocker = IDegenV2LpLocker(lpLocker_);
        poolManager = manager;
    }
        ...
        returns (LaunchResult memory result)
    {
        ...
        emit DegenLaunchConfigured(
            context.token, poolId, firstPositionId, context.beneficiary, context.feeAdmin
        );
    }
}
