...
interface IDegenV2Hook {
    ...
    event ProtocolWethSweepFailed(
        PoolId indexed triggeringPoolId,
        uint256 attemptedTreasuryAmount,
        uint256 attemptedBuybackAmount,
        bytes reason
    );
    ...
    function totalTreasuryWethAccrued(PoolId poolId) external view returns (uint256);
    function totalBuybackWethAccrued(PoolId poolId) external view returns (uint256);
    function pendingTreasuryWeth() external view returns (uint256);
    function pendingBuybackWeth() external view returns (uint256);
    function totalTreasuryWethSwept() external view returns (uint256);
    function totalBuybackWethSwept() external view returns (uint256);
