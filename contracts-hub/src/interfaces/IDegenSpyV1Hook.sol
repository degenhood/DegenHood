...
/// @title SPY V4.1 hook interface
/// @notice Raw-SPY pool registration, fee accounting, and permissionless settlement.
interface IDegenSpyV1Hook {
    struct PoolConfig {
        address token;
        address beneficiary;
    ...
    function totalRawSpyFeesAccrued(PoolId poolId) external view returns (uint256);
    function pendingTreasuryRawSpy(PoolId poolId) external view returns (uint256);
    function pendingBuybackRawSpy(PoolId poolId) external view returns (uint256);
    function pendingBeneficiaryRawSpy(PoolId poolId, address beneficiary)
        external
        view
        returns (uint256);
    function pendingTotalRawSpy(PoolId poolId) external view returns (uint256);
