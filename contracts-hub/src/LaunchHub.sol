...
/// @title LaunchHub
/// @notice Immutable, non-upgradeable kernel for curated DegenHood launch domains.
contract LaunchHub {
    using SignatureChecker for address;
    using SafeCast for uint256;

    bytes32 public constant KERNEL_VERSION = keccak256("LaunchHub/1.0.0");
    bytes32 public constant EMPTY_SCHEMA_HASH = keccak256("EMPTY");
    uint256 public constant DOMAIN_ADMISSION_DELAY = 72 hours;
    TemplateId public constant DEGEN_V1_UNISWAP_V4 = TemplateId.wrap(2);
    TemplateId public constant DEGEN_V1_UNISWAP_V3 = TemplateId.wrap(3);
    TemplateId public constant DEGEN_V1_SPY_UNISWAP_V4 = TemplateId.wrap(4);
    ...
    bytes32 private constant NAME_HASH = keccak256("DegenHoodLaunchHub");
    ...
    constructor(
        ...
        if (
            TemplateId.unwrap(templates[0].id) != TemplateId.unwrap(DEGEN_V1_UNISWAP_V4)
                || TemplateId.unwrap(templates[1].id) != TemplateId.unwrap(DEGEN_V1_UNISWAP_V3)
                || TemplateId.unwrap(templates[2].id) != TemplateId.unwrap(DEGEN_V1_SPY_UNISWAP_V4)
        ) {
            revert InvalidGenesisTemplateSet();
        }
        if (
            templates[0].config.module == templates[1].config.module
                || templates[0].config.module == templates[2].config.module
                || templates[1].config.module == templates[2].config.module
        ...
        _installGenesisTemplate(genesis.id, templates[2]);
